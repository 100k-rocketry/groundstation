...
\begin{document}
	...
	altitude, latitude, longitude, and more. Latitude and longitude
	data is helpful for locating the rocket after it lands, but
	if the data is located on the rocket, then the data is useless
	for this task. Altitude data will tell the rocketry team if we have
	met the objective of one hundred thousand feet, but if the rocket
	is damage or cannot be located for any reason, then the record
	of this achievement may be lost.

	However, the chicken-and-egg problem of not knowing the location
	of the rocket before the rocket is found may be avoided by sending
	telemtry from the rocket in real-time. Data may be received wirelessly
	by the rocketry team while the rocket is in flight, giving them
	access to many useful data, including the rocket's last known location
	and the highest altitude reached.
